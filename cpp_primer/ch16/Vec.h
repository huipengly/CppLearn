...
public:
    ...
    ~Vec() { free(); }
    T& at(std::size_t n) { if (n > size()) throw std::out_of_range("out_of_range"); return elements[n]; }
    const T& at(std::size_t n) const { if (n > size()) throw std::out_of_range("out_of_range"); return elements[n]; }
...
private:
    ...
    void allocate_n_move(const std::size_t);                         //!< 为元素分配内存
    void free();                                                //!< 释放内存
