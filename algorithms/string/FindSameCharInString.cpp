...
string FindSameCharInString(const string &str1, const string &str2)
{
	string ret;
	for (auto c : str1)
		if (str2.find(c) < str2.size())		// string的find是返回距离off的距离，所以应该判断大小
			ret += c;
	return ret;
}
...
int main()
{
	string str1{ "ac" }, str2{ "abcs" };
	cout << FindSameCharInString(str1, str2) << endl;
	//test();
