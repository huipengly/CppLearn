...
这个问题其实是一个特例的遍历，按照右中左的顺序遍历。被遍历的节点的特点是根节点为down，其余的均为左节点为up，右节点为down。

所以不需要构造一个二叉树，只需要按照遍历右节点，输出节点，遍历左节点的想法来完成递归即可。

## 4.10 判断一棵树是否是平衡树

平衡树，每一个节点的左节点高度和右节点高度差小于1。

递归方法，要判断每个节点的左节点是否平衡，右节点是否平衡，左节点高度，右节点高度。所以需要一个函数返回节点的高度和平衡性。

## 4.11 判断一颗树是否是搜索二叉树

搜索二叉树，每个节点的值大于左节点机器子树节点，小于右节点及其子树节点。

最简单的实现：二叉树中序遍历是升序。

递归：给出一个节点的最大值和最小值范围，然后比较。最大值最小值范围和其父节点相关。

## 4.12 判断一颗树是否是完全二叉树

方法1：按层遍历，如果出现一个节点为空，那么之后的所有节点都必须为空节点。

方法2：按层遍历，情况1：如果一个节点有右节点没有左，那么不是完全二叉树。情况2：如果一个节点为叶节点或只有左节点，那么之后所有的节点需要都为叶节点。

## 4.13 计算完全二叉树节点个数

方法1：按层遍历，计数。time：O(n)

方法2：通过右子树的高度和二叉树的高度，判断左子树是满二叉树还是右子树是满二叉树。不满的一边使用递归方法继续。
...
遇到一个1，使用深度优先搜索，将相连的都标记为2，然后岛数目加一。