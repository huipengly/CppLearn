...
// time:O(n), extra space:O(n) 实际n/2
class Solution2 {
public:
	bool isPalindrome(ListNode* head) {
		stack<int> stack;
...
public:
	bool isPalindrome(ListNode* head) {
		...
		auto *rhead = reverseList(curr->next);			// 反转后半部分
		curr->next = nullptr;							// 前后两部分分开
		auto *halfTail = curr;							// 前半部分的尾
		...
		halfTail->next = reverseList(rhead);			// 恢复原始链表
...
int main()
{
	...
	cout << s1.isPalindrome(nullptr) << endl;
