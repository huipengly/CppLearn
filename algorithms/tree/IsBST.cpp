...
private:
	bool isValidBST(TreeNode *root, TreeNode *lowerLimit, TreeNode *upperLimit)
	{
		if (root == nullptr)
			return true;

		if (lowerLimit != nullptr && root->val <= lowerLimit->val)
			return false;
		if (upperLimit != nullptr && root->val >= upperLimit->val)
			return false;

		if (root->left != nullptr && !isValidBST(root->left, lowerLimit, root))
			return false;
		if (root->right != nullptr && !isValidBST(root->right, root, upperLimit))
			return false;
