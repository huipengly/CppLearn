...
public:
	TreeLinkNode* GetNext(TreeLinkNode* pNode)
	{
		...
		if (pNode->right != nullptr)			// 右不为空，则为右最左
		{
			pNode = pNode->right;
		...
		{
			while (pNode->next != nullptr && pNode->next->right == pNode)    // 当前节点是父右，则一直向上找
			{
				pNode = pNode->next;
			}
			pNode = pNode->next;		// 父左的情况，那么父就是下一个中序遍历节点。
		}
		return pNode;
	}
};
...
int main()
{
	...
	auto *l1 = new TreeLinkNode(2);
	root->right = l1;
	l1->next = root;
	Solution s;
	auto *ret = s.GetNext(l1);
